...
The `paldex_data` named volume is mounted only at `/app/data/live` (not the
whole `/app/data`), so it only ever holds the refresh loop's own
`players.json`, `bases.json`, `dungeons_state.json`, and the `data/saves/`
save-file cache — persists across container restarts/redeploys. The
image's baked-in `data/*_static.json` (schematics/relics/bosses/etc.) always
comes straight from whatever image is currently running, never through the
volume, so a rebuilt image's static data takes effect immediately with no
volume management at all. **`data/live/` contains real player names/
...
(new schematics/effigies/etc. from a game patch), commit the refreshed
`data/*_static.json` + `frontend/assets/` icons and rebuild/redeploy the
image — since the `paldex_data` volume only covers `data/live/`, the new
static JSON is live the moment the new image starts. No volume management
needed.
