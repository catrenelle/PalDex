...
# backend/BUILD_VERSION (date-based app build stamp, not the Palworld game
# server's own version already surfaced via /api/players' server_version)
# is a tracked file regenerated by .githooks/pre-commit at commit time, not
# here at image build time — reproducible across whoever/wherever builds a
# given commit, and avoids needing .git in the build context.
